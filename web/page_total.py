...
def _build_contract_breakdown_by_salesperson() -> dict[str, pd.DataFrame]:
    ...
    # (销售员, 合同编号) -> 销售部门：合同归属部门，支持跨部门销售员按部门取档位。
    contract_dept_map = get_contract_dept_map()

    # 完成额度档位率按 (销售员, 销售部门) 取：跨部门销售员各部门各自一档。
    quota_rate_map: dict[tuple[str, str], float] = {}
    if quota_df is not None and not quota_df.empty and "销售员" in quota_df.columns:
        for _, r in quota_df.iterrows():
            sp = str(r.get("销售员", "")).strip()
            if not sp:
                continue
            dept = str(r.get("销售部门", "") or "").strip()
            quota_rate_map[(sp, dept)] = _parse_pct_to_ratio(r.get("提成比例", 0))
...
def _build_contract_breakdown_flat(total_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for sp, df in breakdown.items():
        for _, r in df.iterrows():
            rows.append({
                "销售员": sp,
                "销售部门": str(r.get("销售部门", "") or "") or dept_map.get(sp, ""),
...
def _build_export_template_df(total_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 销售部门与完成额度系数直接取自合同级明细（flat 已按合同归属部门算好），
    # 不再按"每人单一部门/单一档位"反推，避免跨部门销售员串档。
...
def _current_total_signature() -> int:
    return calc_version()


def _valid_total_result() -> pd.DataFrame | None:
    total_df = st.session_state.get("total_result")
    if total_df is None or total_df.empty:
        return total_df
    if st.session_state.get("total_result_signature") != _current_total_signature():
        return None
    return total_df
...
def render_total(username: str):
    ...
    total_df = _valid_total_result()
    if stored_total_df is not None and not stored_total_df.empty and total_df is None:
        st.info("提成计算结果已更新，请重新点击“汇总计算”生成最新总汇总。")
