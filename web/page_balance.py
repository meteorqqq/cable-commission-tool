...
def _build_balance_df(
    ...
    if delivery_df is not None and not delivery_df.empty:
        ...
        for (pid, sp), g in dd.groupby(["合同编号", "销售员"]):
            ...
            if "销售部门" in g.columns and "销售部门" not in rows[key]:
                rows[key]["销售部门"] = _first_clean(g["销售部门"])
    ...
    # 完成比提成比例按 (销售员, 销售部门) 取：跨部门销售员每个部门各自一档。
    quota_rate_map: dict[tuple[str, str], str] = {}
    if quota_df is not None and not quota_df.empty and "销售员" in quota_df.columns:
        for _, r in quota_df.iterrows():
            ...
            quota_rate_map[(sp, dept)] = str(r.get("提成比例", "") or "")
    ...
    for (pid, sp), v in rows.items():
        ...
        out.append({
            ...
            "完成比提成比例": quota_rate_map.get((str(sp).strip(), dept), ""),
...
def _parse_opening_excel(path: str) -> pd.DataFrame:
    """宽松解析期初结余 Excel / CSV：匹配本页导出的列名，也兼容简写。"""
    ...
    # 部门做编号清洗，保证期初行的部门名与当期数据一致（否则
    # "020201|02-国网事业部" 会和当期清洗后的 "国网事业部" 算成两个部门）
    out["销售部门"] = (
        raw[col_map["销售部门"]].map(clean_dept_name)
...
def render_opening_balance_import() -> None:
    ...
    with c_up:
        uploaded = st.file_uploader(
            "期初结余 Excel / CSV", type=["xls", "xlsx", "csv"], key="opening_uploader",
        )
    with c_date:
