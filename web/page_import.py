...
UPLOAD_FINGERPRINT_KEYS = (
    "delivery_uploader_fingerprint",
    "payment_uploader_fingerprint",
)


def _uploaded_fingerprint(uploaded) -> str:
    pos = uploaded.tell()
    uploaded.seek(0)
    data = uploaded.read()
    uploaded.seek(pos)
    digest = hashlib.md5(data).hexdigest()
    return f"{uploaded.name}:{getattr(uploaded, 'size', len(data))}:{digest}"
...
def _upload_and_load(
    ...
    if uploaded is not None:
        ...
        fingerprint = _uploaded_fingerprint(uploaded)
        if st.session_state.get(fingerprint_key) == fingerprint and st.session_state.get(state_key) is not None:
            st.caption("文件未变化，已复用当前导入数据。")
        else:
