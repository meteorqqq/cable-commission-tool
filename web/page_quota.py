...
def _sum_by_dept(df, amount_col: str) -> dict[str, float]:
    """按每行自身的销售部门汇总金额（万元）。

    跨部门销售员的行各自归到本行记录的部门，不再经 销售员→单一部门 映射，
    与引擎 ``calc_quota_commission_by_dept`` 的部门口径保持一致。
    """
    if df is None or "销售部门" not in df.columns or amount_col not in df.columns:
        return {}
    amounts = pd.to_numeric(df[amount_col], errors="coerce").fillna(0)
    totals: dict[str, float] = {}
    for dept, grp in amounts.groupby(df["销售部门"].astype(str)):
        d = str(dept).strip()
        if d and d.lower() not in ("nan", "none"):
            totals[d] = round(grp.sum() / 10000, 2)
    return totals


def _calc_dept_totals() -> dict[str, float]:
    """各部门回款额（万元），作为目标额的参考默认值。"""
    return _sum_by_dept(st.session_state.get("payment_df"), "回款金额")


def _calc_dept_delivery_totals() -> dict[str, float]:
    """各部门发货额（万元），只读参考，与引擎"部门实际发货"同口径。"""
...
def render_quota(username: str):
    ...
    if st.button("计算完成额度提成", type="primary", use_container_width=True):
        ...
        try:
            ...
            st.success(f"计算完成，共 {n_sp} 位销售员{extra}")
    ...
    if result is not None and not result.empty:
        with st.container(border=True):
            ...
            total_sp = int(result["销售员"].astype(str).nunique())
            ...
            _has_commission = pd.to_numeric(
                result.get("完成额度提成(元)", 0), errors="coerce"
            ).fillna(0).abs() > 0.005
            commissioned_n = int(
                result.loc[_has_commission, "销售员"].astype(str).nunique()
            )
            with m1:
