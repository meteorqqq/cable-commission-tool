...
def render_salesperson():
    ...
    dept_list = detail.get("部门列表") or (
        [detail["销售部门"]] if detail.get("销售部门") else []
    )
    c1, c2, c3, c4, c5, c6 = st.columns(6, gap="medium")
    with c1:
        if len(dept_list) > 1:
            # 跨部门销售员：指标显示部门数，完整部门列表放进 help 提示
            st.metric("销售部门", f"{len(dept_list)} 个部门",
                      help=" / ".join(dept_list))
        else:
