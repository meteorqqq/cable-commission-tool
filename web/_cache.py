...
def get_contract_dept_map() -> dict[tuple[str, str], str]:
    """{(销售员, 合同编号) -> 销售部门}，按合同归属部门，支持跨部门销售员。"""
    from engine.calculator import build_contract_dept_map

    @session_cache("contract_dept_map", scope="data")
    def _compute():
        return build_contract_dept_map(
            st.session_state.get("delivery_df"),
            st.session_state.get("payment_df"),
        )
