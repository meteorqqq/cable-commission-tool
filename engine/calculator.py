...
def _read_table(path: str, header=0, nrows: int | None = None) -> pd.DataFrame:
    """按扩展名分发读取 Excel 或 CSV。

    上传入口同时允许 .xls/.xlsx/.csv，但解析必须随格式切换，否则给
    ``pd.read_excel`` 喂 CSV 会直接报 "Excel file format cannot be determined"。
    中文 CSV 常见 GBK/UTF-8(BOM) 编码，这里依次尝试，最后用 latin-1 兜底。
    """
    if str(path).lower().endswith(".csv"):
        last_err: Exception | None = None
        for enc in ("utf-8-sig", "gbk", "utf-8"):
            try:
                return pd.read_csv(path, header=header, nrows=nrows, encoding=enc)
            except UnicodeDecodeError as e:
                last_err = e
                continue
        # latin-1 不会抛 UnicodeDecodeError，作为最终兜底
        try:
            return pd.read_csv(path, header=header, nrows=nrows, encoding="latin-1")
        except Exception:
...
def _detect_header_row(path: str) -> int:
    probe = _read_table(path, header=None, nrows=10)
...
def _is_contract_id_col(normalized_col: str) -> bool:
    """判断（去空格后的）列名是否为合同标识列。

    用「项目」「工程」子串覆盖 项目名称 / 项目号 / 项目编号 / 工程名称 等写法；
    刻意不匹配裸「合同」，以免把价格列「合同价 / 合同总价」误判为标识列。
    """
    if "合同编号" in normalized_col or "合同号" in normalized_col:
        return True
...
def load_contract_pricing_excel(path: str) -> dict[str, "ContractPricing"]:
    ...
    支持的列别名（模糊匹配，忽略空白和"（元）"等后缀）：
      - 合同标识：合同编号 / 合同号 / 项目名称 / 项目号 / 项目编号 / 工程名称
        （凡列名含「项目」或「工程」均按合同标识列处理；其值需与发货/回款的
        合同编号一致才能join上）
      - 指导价 / 基准价 / 目标价 / 标准价 / 参考价
    ...
    if pid_col is None:
        raise ValueError(
            "未找到合同标识列（合同编号/合同号/项目名称/项目号/工程名称）。实际表头为："
            f"{list(df.columns)}"
        )
...
def build_contract_overview(delivery_df: pd.DataFrame | None,
    ...
    pids: set[str] = set()
    delivery_summary: dict[str, dict] = {}
    payment_summary: dict[str, dict] = {}

    if delivery_df is not None and not delivery_df.empty and "合同编号" in delivery_df.columns:
        tmp = delivery_df.copy()
        tmp["合同编号"] = tmp["合同编号"].astype(str)
        tmp = tmp[delivery_df["合同编号"].notna()]
        if not tmp.empty:
            if "发货金额" in tmp.columns:
                tmp["_金额"] = pd.to_numeric(tmp["发货金额"], errors="coerce").fillna(0)
            else:
                tmp["_金额"] = 0.0
            pids.update(tmp["合同编号"].unique())
            agg = tmp.groupby("合同编号").agg(
                交货行数=("合同编号", "size"),
                交货金额合计=("_金额", "sum"),
                含退货=("_金额", lambda s: bool((s < -_AMOUNT_EPS).any())),
            )
            delivery_summary = agg.to_dict("index")

    if payment_df is not None and not payment_df.empty and "合同编号" in payment_df.columns:
        tmp = payment_df.copy()
        tmp["合同编号"] = tmp["合同编号"].astype(str)
        tmp = tmp[payment_df["合同编号"].notna()]
        if not tmp.empty:
            if "回款金额" in tmp.columns:
                tmp["_金额"] = pd.to_numeric(tmp["回款金额"], errors="coerce").fillna(0)
            else:
                tmp["_金额"] = 0.0
            pids.update(tmp["合同编号"].unique())
            agg = tmp.groupby("合同编号").agg(
                回款行数=("合同编号", "size"),
                回款金额合计=("_金额", "sum"),
                含退款=("_金额", lambda s: bool((s < -_AMOUNT_EPS).any())),
            )
            payment_summary = agg.to_dict("index")

    rows = []
    for pid in sorted(pids):
        d_info = delivery_summary.get(pid, {})
        p_info = payment_summary.get(pid, {})
        d_lines = int(d_info.get("交货行数", 0))
        p_lines = int(p_info.get("回款行数", 0))
        if d_lines > 0 and p_lines > 0:
            ...
            tag = "仅回款明细"
        business_flags = []
        if bool(d_info.get("含退货", False)):
            business_flags.append("有退货")
        if bool(p_info.get("含退款", False)):
            business_flags.append("有退款")
        rows.append({
            "合同编号": pid,
            "交货行数": d_lines,
            "交货金额合计": round(float(d_info.get("交货金额合计", 0)), 2),
            "回款行数": p_lines,
            "回款金额合计": round(float(p_info.get("回款金额合计", 0)), 2),
...
def build_salesperson_detail(
    ...
    支持跨部门销售员：``销售部门`` 为该销售员涉及的所有部门拼接（" / " 连接），
    ``部门列表`` 给出去重排序后的部门数组；每个合同另带自身归属的 ``销售部门``。

    结构:
        {
            "销售员": str,
            "销售部门": str,          # 多部门时为 "A / B"
            "部门列表": list[str],    # 去重排序，如 ["东北大区", "海外事业部"]
    ...
    """
    # 部门按"合同归属"取，支持同一销售员横跨多个部门；
    # 找不到合同级部门时回落到该销售员的整体部门。
    dept_map = _build_salesperson_dept_map(delivery_df, payment_df)
    fallback_dept = dept_map.get(salesperson, "")
    ...
    # (销售员, 合同编号) → 部门：用于给每个合同标注归属部门并汇总该销售员的部门集合
    contract_dept = build_contract_dept_map(del_rows, pay_rows)
    ...
    for pid in sorted(pids, key=_order_key):
        ...
        c_dept = contract_dept.get((salesperson, str(pid)), fallback_dept)
        if c_dept:
            dept_set.add(c_dept)
    ...
    dept = " / ".join(dept_list) if dept_list else fallback_dept
...
def build_contract_dept_map(
    delivery_df: pd.DataFrame | None,
    payment_df: pd.DataFrame | None = None,
) -> dict[tuple[str, str], str]:
    """构建 (销售员, 合同编号) → 销售部门 映射。

    部门是「合同 / 数据行」的属性而非「人」的属性：同一销售员的不同合同可能
    分属不同部门（如海外事业部 + 区域营销部）。这里按每行自身的销售部门归集，
    交货优先于回款；同一 (销售员, 合同) 多次出现时取首个非空部门。

    需要"按合同显示部门"的场景（利润提成 / 回款时效 / 销售员详情 / 结余）都应
    用本映射，而不是 :func:`build_salesperson_dept_map` 的"一人一部门"近似。
    """
    mapping: dict[tuple[str, str], str] = {}
    for src in (delivery_df, payment_df):
        if src is None or getattr(src, "empty", True):
            continue
        if not {"销售员", "合同编号", "销售部门"}.issubset(src.columns):
            continue
        sub = src[["销售员", "合同编号", "销售部门"]].astype(str)
        for sp, pid, dept in sub.itertuples(index=False):
            d = dept.strip()
            if not d or d.lower() in ("nan", "none"):
                continue
            mapping.setdefault((sp.strip(), pid.strip()), d)
    return mapping
...
def calc_quota_commission_by_dept(delivery_df: pd.DataFrame,
    ...
    个人发货/回款额按每行自身记录的部门归集。绝不能把某部门的发货并入该
    销售员"首次出现"的部门，否则两个部门的完成比都会被污染。
    """
    if tiers is None:
    ...
    def _grp(df: pd.DataFrame | None, amount_col: str, out_col: str) -> pd.DataFrame:
        ...
        if (df is None or getattr(df, "empty", True)
            ...
            return pd.DataFrame(columns=cols)
        # 部门只做 strip，不做编号清洗：与页面 _get_dept_list / dept_targets
        # 的口径保持一致（正常导入时数据已在加载阶段清洗过编号前缀）。
        work = pd.DataFrame({
    ...
    out["_dept_sort"] = out["销售部门"].astype(str)
    out["_sp_sort"] = out["销售员"].astype(str)
    return (out.sort_values(["_dept_sort", "_sp_sort"])
            .drop(columns=["_dept_sort", "_sp_sort"]).reset_index(drop=True))
...
def calc_profit_commission(delivery_df: pd.DataFrame,
    ...
    dept_map = _build_salesperson_dept_map(delivery_df, payment_df)
    contract_dept = build_contract_dept_map(delivery_df, payment_df)
...
def calc_payment_timeliness(delivery_df: pd.DataFrame,
    ...
    def _dept_of(sp, pid) -> str:
        """按 (销售员, 合同编号) 取归属部门，回落到该销售员的整体部门。"""
        return contract_dept.get((str(sp), str(pid)), dept_map.get(sp, "")) or ""
    ...
    del_summary["销售部门"] = [
        _dept_of(sp, pid)
        for sp, pid in zip(del_summary["销售员"], del_summary["合同编号"])
    ]
    ...
    pay_summary["销售部门"] = [
        _dept_of(sp, pid)
        for sp, pid in zip(pay_summary["销售员"], pay_summary["合同编号"])
    ]
    ...
    for (salesperson, project), grp_pay in payment_df.groupby(["销售员", "合同编号"]):
        ...
        # 超出发货额的"过量回款"栈：重复付款 / 超付等，本身未计提成。
        # 退款时优先冲这里，避免把已合理计提的提成误冲为 0。
        # 每项：[回款日, 剩余可冲销金额]
        overpay_stack: list[list] = []
        ...
        for _, pr in grp_pay.sort_values("回款日期").iterrows():
            ...
            if pay_amount > 0.01:
                ...
                if pay_amount > 0.01:
                    overpay_stack.append([pay_date, pay_amount])
                    timeliness_rows.append({
                        "合同编号": project, "销售员": salesperson, "销售部门": dept,
                        "客户单位": customer_unit,
            ...
            # ── 负回款（退款）：先冲"超出发货额"的过量回款，再 LIFO 冲销已匹配提成 ──
            elif pay_amount < -0.01:
                need = -pay_amount  # 待冲销正数

                # ① 先冲减"超出发货额"的过量回款（重复付款 / 超付）。
                #    这部分本就没产生提成，退款冲它不影响已合理计提的提成。
                while need > 0.01 and overpay_stack:
                    o_avail = overpay_stack[-1][1]
                    take = min(need, o_avail)
                    timeliness_rows.append({
                        "合同编号": project, "销售员": salesperson, "销售部门": dept,
                        "客户单位": customer_unit,
                        ...
                        "匹配发货日期": None, "回款周期(天)": None,
                        "业务类型": "退款",
                        "时效提成比例": "超出发货额退款冲销", "时效提成金额": 0,
                    })
                    overpay_stack[-1][1] -= take
                    if overpay_stack[-1][1] < 0.01:
                        overpay_stack.pop()
                    need -= take

                # ② 再 LIFO 冲销之前已匹配（计提过提成）的记录
